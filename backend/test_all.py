    ...
    for cnt in contours:
        ...
        while i < n - 1:
            ...
            for k in range(3, min(60, n - i)):
                ...
                # Calculate curve length to rigidly prevent >180 degree SVG overlaps
                curve_length = np.sum(np.sqrt(np.diff(x)**2 + np.diff(y)**2))
                if curve_length > r * 2.0:
                    break # Stop growing! Arc shouldn't exceed ~114 degrees to prevent SVG swirl
                    
                distances = np.sqrt((x - cx)**2 + (y - cy)**2)
                max_err = np.max(np.abs(distances - r))
                
                if max_err <= tol:
                    # Enforce stable bisectors
                    if dist_span < r * 0.5:
                        continue
            ...
            if best_k >= 3 and best_arc:
                ...
                # Since curve_length is restricted to < 180 degrees, large_arc is ALWAYS 0!
                large_arc = 0
...
test_dir = '/home/haku/projects/DHQ_Root/test_files'
output_dir = '/home/haku/projects/DHQ_Root/backend'

for file_name in ["circle1.png", "Yin-Yang-Logo.png", "GI.png"]:
    in_path = os.path.join(test_dir, file_name)
    paths, w, h = trace_to_svg(in_path, epsilon_val=0.5, tol=1.0)
    
    out_path = os.path.join(output_dir, f"out_{file_name}.svg")
    with open(out_path, 'w') as f:
        f.write(f'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {w} {h}">\n')
        # Add a white background just to be clean
        f.write(f'<rect width="{w}" height="{h}" fill="white"/>\n')
        for p in paths: 
            f.write(f'<path d="{p}" fill="none" stroke="black"/>\n')
        f.write('</svg>\n')
    print(f"Generated {out_path} with {len(paths)} paths.")
